...
bool verify_structure(Graph *g, uint32_t num_nodes, uint32_t num_edges, int trials) {
      ...
      vector<uint32_t> r3 = matrix.bfs(start_node);
      ...
      vector<uint32_t> r5 = matrix.pagerank(test_vector2);
      vector<uint32_t> r6 = g->pagerank(test_vector2);
      if (r5 != r6) {
...
bool time_structure_real_world_graphs(Graph* g, int graph_num) {
  ...
  if (myfile.is_open()) {
    while (getline(myfile, line)) {
      ...
      vals.push_back(rand_in_range(MAXVAL));
    }
    myfile.close();
    // return 0;
  } else {
